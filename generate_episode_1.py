#!/usr/bin/env python3
"""
Generate Episode 1 for all 10 premium newsletters
Each with unique, high-value content for their target audience
"""
...
# Episode 1 content for each newsletter
EPISODE_1_CONTENT = {
...
def create_episode_1():
    """Create Episode 1 for all newsletters."""

    console.print("[bold cyan]📰 GENERATING EPISODE 1 FOR ALL NEWSLETTERS[/bold cyan]\n")
    ...
    try:
        ...
        for newsletter in newsletters:
            ...
            content_key = None
            for key in EPISODE_1_CONTENT.keys():
                if key in newsletter.name:
                    content_key = key
                    break

            if not content_key:
                console.print(f"[yellow]⚠️  No Episode 1 content for: {newsletter.name}[/yellow]")
                continue

            content = EPISODE_1_CONTENT[content_key]

            # Check if Episode 1 already exists
            existing = session.query(Edition).filter(
                Edition.newsletter_id == newsletter.id,
                Edition.subject.like("%Episode 1%")
            ).first()

            if existing:
                console.print(f"[dim]Episode 1 already exists for: {newsletter.name}[/dim]")
                continue

            # Create Episode 1
            edition = Edition(
                newsletter_id=newsletter.id,
                subject=f"Episode 1: {content['subject']}",
                content=json.dumps({
                    "episode": 1,
                    "subject": content['subject'],
                    "executive_summary": content['executive_summary'],
                    "main_insights": content['main_insights'],
                    "key_metrics": content.get('key_metrics') or
                                   content.get('portfolio_breakdown') or
                                   content.get('growth_metrics') or
                                   content.get('cost_comparison', {}),
                    "preview_text": content['executive_summary'][0]
                }),
            ...
            console.print(f"[green]✅ Created Episode 1 for: {newsletter.name}[/green]")
            console.print(f"   [dim]{content['subject']}[/dim]\n")

        session.commit()

        console.print(f"\n[bold green]✨ Created {created_count} Episode 1 editions![/bold green]")

        # Save Episode 1 content to file for reference
        output_file = Path("episode_1_content.json")
        with open(output_file, "w") as f:
            json.dump(EPISODE_1_CONTENT, f, indent=2)

        console.print(f"[cyan]📄 Full content saved to: {output_file}[/cyan]")
